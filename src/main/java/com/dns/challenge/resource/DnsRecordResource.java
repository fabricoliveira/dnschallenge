...
public class DnsRecordResource {
	...
	@GetMapping("/{id}")
	public ResponseEntity<DnsRecordDTO> findById(@PathVariable Long id) throws Exception {
		
		if(id < 0)
			throw new InvalidInputException("Must have a valid idenfier.");
		
		DnsRecordDTO result = dnsRecordService.findByIdJoinIdJoinHostname(id);
